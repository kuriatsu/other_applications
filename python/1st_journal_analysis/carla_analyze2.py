...
rank_df = pd.read_csv('/run/media/kuriatsu/SamsungKURI/master_study_bag/202102experiment/result/rank.csv')
with open('/run/media/kuriatsu/SamsungKURI/master_study_bag/202102experiment/profile.pickle', 'rb') as f:
...
for experiment_type in summary_df.experiment_type.drop_duplicates():
    for subject in summary_df.subject.drop_duplicates():
        ...
        buf = pd.DataFrame([[subject, experiment_type,
            len(target_df[(target_df.actor_action == "pose") & (target_df.min_vel < 1.0)]),
            len(target_df[(target_df.actor_action == "pose") & (target_df.min_vel >= 1.0)]),
            len(target_df[(target_df.actor_action == "cross") & (target_df.min_vel < 1.0)]),
            len(target_df[(target_df.actor_action == "cross") & (target_df.min_vel >= 1.0)]),
            len(target_df[((target_df.actor_action == "pose") & (target_df.min_vel < 1.0)) | ((target_df.actor_action == "cross") & (target_df.min_vel >= 10.0))])/len(target_df),
...
axes.set_ylabel('Driving with Higher Velocity Than Threshold', fontsize=12)

axes.legend(loc='lower left', fontsize=12)

################################################################
print("min_Speed stacked graph in cross scenario")
################################################################
fig, axes = plt.subplots()
min_vel_df_cross = pd.DataFrame(columns=["Rate", "Threshold", "experiment_type"])
for i, type in enumerate(experiments):
    target_df = summary_df[(summary_df.experiment_type == type) & (summary_df.actor_action == "cross")]
    buf_df = pd.DataFrame(
        [[
            len(target_df[target_df.min_vel>-1]) / len(target_df),
            0,
            type,
        ]],
        columns=min_vel_df_cross.columns
    )
    min_vel_df_cross = pd.concat([min_vel_df_cross, buf_df], ignore_index = True)
    for thresh in range(1, 50):
        buf_df = pd.DataFrame(
            [[
                len(target_df[target_df.min_vel>thresh]) / len(target_df),
...
# min_vel_df_cross = min_vel_df_cross.drop(min_vel_df_cross.index[min_vel_df_cross.Threshold == 0])
# min_vel_df_cross.loc[min_vel_df_cross.Threshold == -1, "Threshold"] = 0
axes = sns.lineplot(data=min_vel_df_cross, x='Threshold', y='Rate', hue="experiment_type", marker='o')
axes.set_ylim(0, 1)
axes.set_xlim([0,50])
axes.set_xlabel('Threshold [km/h]', fontsize=15)
axes.set_ylabel('Driving with Higher Velocity Than Threshold', fontsize=12)
...
################################################################
print("stop_distance stacked graph in cross scenario")
################################################################
fig, axes = plt.subplots()
min_vel_df_cross = pd.DataFrame(columns=["Rate", "Threshold", "experiment_type"])
for i, type in enumerate(experiments):
    for thresh in range(0, 30):
        ...
        buf_df = pd.DataFrame(
            [[
                len(target_df[target_df.stop_distance>thresh]) / len(target_df),
                thresh,
                type,
            ]],
            columns=min_vel_df_cross.columns
...
axes.set_ylim(0, 1)
axes.set_xlim([0,30])
axes.set_xlabel('Threshold [m]', fontsize=15)
axes.set_ylabel('Rate of Driving with \n(Stoped Distance > Threshold)', fontsize=12)
