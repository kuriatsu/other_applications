...
def alignData(result, annotations_tree, ego_vehicle_tree, attributes_tree):
    ...
    for result_row in result:
        ...
        if row[3] == 'pedestrian':
            ...
            # appear bounding box -> intervene
            distance = 0
            for ego_vehicle in ego_vehicle_tree[int(row[8]):int(row[13])]:
                distance += float(ego_vehicle.attrib.get('GPS_speed'))  * 0.03 / 3.6

            row += [
                ego_vehicle_tree[int(row[8])].attrib.get('GPS_speed'),
                distance
            ]
...
def writeCsv(pedestrian_data, tr_data, filename):
    ...
    with open(filename, 'a') as file_obj:
        ...
        # writer.writerow(['displayed_frame', 'display_time', 'id', 'obj_type', 'prob', 'framein_point', 'frameout_point', 'intervene_type', 'intervene_frame', 'intervene_time', 'intervene_key',
...
# def calcData(filename, alignData):
...
def main():
    ...
    default='/home/kuriatsu/share/PIE_result/20200626_yokoyama/yokoyama_1_enter.csv'
    )
    argparser.add_argument(
    '--annotations', '-anno',
    ...
    default='/home/kuriatsu/share/PIE_result/test/result.csv'
    )
    ...
    print(args.result.split('.')[0].split('_')[-1])
